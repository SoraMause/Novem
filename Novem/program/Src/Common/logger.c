...
{
  printf( "log start\r\n" );
  printf( "sen l, sen r, sen f, ideal vel, real vel, ideal_dis, ideal rota, real rota, batt\r\n");
  for ( int i  = 0; i < log_count; i++ ){
    printf( "%d,%d,%d,%d,%d,%f,%d,%d,%d\r\n",logger.sensor_left[i], logger.sensor_right[i],
            logger.sensor_front[i], logger.trans_ideal_vel[i], logger.trans_vel[i],logger.trans_dis[i],
